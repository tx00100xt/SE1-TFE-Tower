...
enum ProjectileType {
 ...
 58 PRT_HEADMAN_NAFIL		  "Nafil Rocket",  // a nafil's rocket
 59 PRT_SPIKEBALL			  "Spikeball",   // player spikeball
};
...
enum ProjectileMovingType {
  ...
  3 PMT_GUIDED_RANDOM "",	  // slightly random guided missile
};
...
void CProjectile_OnPrecache(CDLLEntityClass *pdec, INDEX iUser) 
{
  ...
  case PRT_SPIKEBALL:
	pdec->PrecacheModel(MODEL_SPIKEBALL);
	pdec->PrecacheTexture(TEXTURE_SPIKEBALL);
	pdec->PrecacheSound(SOUND_SPIKEBALL_BOUNCE);
	break;
...
// **** PLAYER SPIKEBALL ****
220 model	MODEL_SPIKEBALL				"Models\\Weapons\\GrenadeLauncher\\Spikeball\\Spikeball.mdl",
221 texture TEXTURE_SPIKEBALL			"Models\\Weapons\\GrenadeLauncher\\Spikeball\\Spikeball.tex",
222 sound   SOUND_SPIKEBALL_BOUNCE      "Models\\Weapons\\GrenadeLauncher\\Sounds\\Bounce.wav",
...
functions:
  ...
  void RenderParticles(void) {
    switch (m_prtType) {
	  ...
	  case PRT_HEADMAN_NAFIL:
		{
		  Particles_RocketTrail(this, 2.0f);
		  break;
		}
      case PRT_GRENADE: {
...
void PlayerRocket(void) {
  ...
  // if a player launched this rocket
  if ( IsDerivedFromClass( m_penLauncher, "Player") )
  {
	  // check whether he's enraged
	  if ( ((CPlayer&)*m_penLauncher).m_bUnderRage )
	  {
		m_fDamageAmount*= 3;
	  }
  }
  ...
  // set up target for projectile
  // can be either targeting or not
  CEntityPointer pw;
...
// player spikeball
void PlayerSpikeball(void)
{
  // set appearance
  InitAsModel();
  SetPhysicsFlags(EPF_MODEL_BOUNCING);
  SetCollisionFlags(ECF_PROJECTILE_SOLID);
  SetModel(MODEL_SPIKEBALL);
  SetModelMainTexture(TEXTURE_SPIKEBALL);
  // start moving
  LaunchAsFreeProjectile(FLOAT3D(0.0f, 2.0f, -m_fSpeed), (CMovableEntity*)&*m_penLauncher);
  SetDesiredRotation(ANGLE3D(0, FRnd()*120.0f+120.0f, FRnd()*250.0f-125.0f));
  en_fBounceDampNormal   = 0.75f;
  en_fBounceDampParallel = 0.6f;
  en_fJumpControlMultiplier = 0.0f;
  en_fCollisionSpeedLimit = 45.0f;
  en_fCollisionDamageFactor = 50.0f;
  m_fFlyTime = 15.0f;
...
void PlayerSpikeballExplosion(void) {
 ...
 *                   HEADMAN PROJECTILE                     *
 ************************************************************/
void NafilRocket(void) {
  // set up guided target
...
void NafilRocketExplosion(void) {
  ESpawnEffect ese;
  FLOAT3D vPoint;
  FLOATplane3D vPlaneNormal;
  FLOAT fDistanceToEdge;
...
procedures:
  ...
  // guided projectile- slighty random, to make things more interesting
  ProjectileRandomGuidedFly(EVoid) {
    // if already inside some entity
    CEntity *penObstacle;
    if (CheckForCollisionNow(0, &penObstacle)) {
      // explode now
    ...
    while( _pTimer->CurrentTick()<(m_fStartTime+m_fFlyTime))
    {
      ...
      if (m_penTarget!=NULL) {
        ...
        FLOAT fSpeedDecreasingFactor = ((180-abs((SLONG)aWantedHeading))/180.0f);
        ...
        if( abs( (SLONG)aWantedHeading) < 30.0f)
        {
  ...
  Main(ELaunchProjectile eLaunch) {
    ...
    switch (m_prtType) {
      ...
      case PRT_GRENADE:
	  case PRT_SPIKEBALL: Particles_GrenadeTrail_Prepare(this); break;
    ...
    case PRT_WALKER_ROCKET: WalkerRocket(); break;
	  ...
	  case PRT_HEADMAN_NAFIL: NafilRocket(); break;
	  case PRT_SPIKEBALL: PlayerSpikeball(); break;
	...
	else if (m_pmtMove==PMT_GUIDED_RANDOM)
	{
	  autocall ProjectileRandomGuidedFly() EEnd;
	}
    ...
    switch (m_prtType) {
	  ...
	  case PRT_HEADMAN_NAFIL: NafilRocketExplosion(); break;
	  case PRT_SPIKEBALL: PlayerSpikeballExplosion(); break;
    }
