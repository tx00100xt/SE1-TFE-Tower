...
components:
 ...
 30 texture TEXTURE_NAFIL			"Models\\Enemies\\Headman\\Nafil.tex",
...
functions:
  ...
  virtual CTString GetPlayerKillDescription(const CTString &strPlayerName, const EDeath &eDeath)
  {
    ...
    } else if (m_hdtType==HDT_NAFIL) {
	  str.PrintF(TRANS("%s was blown to smithereens by a Nafil"),(const char*) strPlayerName);
	}
    return str;
  }
  ...
  virtual const CTFileName &GetComputerMessageName(void) const {
	...
	static DECLARE_CTFILENAME(fnmNafil,		  "Data\\Messages\\Enemies\\Nafil.txt");
    switch(m_hdtType) {
	...
	case HDT_NAFIL:		  return fnmNafil;
    }
  };
  ...
  void Precache(void) {
	...
	case HDT_NAFIL:		  {
	  PrecacheClass(CLASS_PROJECTILE, PRT_HEADMAN_NAFIL);
      PrecacheSound(SOUND_FIREROCKETMAN);
						  } break;
  ...
  BOOL FillEntityStatistics(EntityStats *pes)
  {
	...
	case HDT_NAFIL:		  { pes->es_strName+=" Nafil"; } break;
    }
    return TRUE;
  }
    ...
    FLOAT fDamageAmmount, const FLOAT3D &vHitPoint, const FLOAT3D &vDirection) 
  {
    ...
    if (!IsOfClass(penInflictor, "Headman") || 
        !(
		  ...
		  ((CHeadman*)penInflictor)->m_hdtType==HDT_NAFIL		  
		 )) {
 ...
 ************************************************************/
  ...
  NafilAttack(EVoid) {
    ...
    StartModelAnim(HEADMAN_ANIM_ROCKETMAN_ATTACK, 0);
	// missiles flurry
    ShootProjectile(PRT_HEADMAN_NAFIL, FLOAT3D( 0.1f, 3.0f, 0.0f), ANGLE3D(0, 0, 0));
    ShootProjectile(PRT_HEADMAN_NAFIL, FLOAT3D(-0.1f, 3.0f, 0.0f), ANGLE3D(0, 0, 0));
    ShootProjectile(PRT_HEADMAN_NAFIL, FLOAT3D( 0.0f, 3.0f, 0.0f), ANGLE3D(0, 0, 0));
    ShootProjectile(PRT_HEADMAN_NAFIL, FLOAT3D( 0.1f, 2.5f, 0.0f), ANGLE3D(0, 0, 0));
    ShootProjectile(PRT_HEADMAN_NAFIL, FLOAT3D(-0.1f, 2.5f, 0.0f), ANGLE3D(0, 0, 0));
    PlaySound(m_soSound, SOUND_FIREROCKETMAN, SOF_3D);  // <-- [TEST] should be changed to other sound
 ...
 ************************************************************/
  Main(EVoid) {
	...
	if(m_hdtType == HDT_NAFIL)
	{
	  SetHealth(250.0f*m_fHPMultiplier);
	  m_fMaxHealth= 250.0f*m_fHPMultiplier;
	}
	else
	{
	  SetHealth(19.5f*m_fHPMultiplier);
      m_fMaxHealth = 19.5f*m_fHPMultiplier;
	}
    ...
    switch (m_hdtType) {
      ...
      case HDT_NAFIL:
        // set your texture
        SetModelMainTexture(TEXTURE_NAFIL);
        AddAttachment(HEADMAN_ATTACHMENT_HEAD, MODEL_FIRECRACKERHEAD, TEXTURE_FIRECRACKERHEAD);
        AddAttachment(HEADMAN_ATTACHMENT_CHAINSAW, MODEL_CHAINSAW, TEXTURE_CHAINSAW);
        // setup moving speed
        m_fWalkSpeed = FRnd() + 1.5f;
