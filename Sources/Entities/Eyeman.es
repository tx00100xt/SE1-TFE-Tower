...
components:
  ...
  6 texture TEXTURE_EYEMAN_ELITE  "Models\\Enemies\\Eyeman\\EyemanElite.tex",
...
functions:
  ...
  {
	// elite eyeman unaffected by impact damage
	if(m_EecChar==EYC_ELITE)
	{
		if(dmtType==DMT_IMPACT)
		{
		  return;
		}
		else if (dmtType==DMT_CLOSERANGE)
		{
		  fDamageAmmount= fDamageAmmount/10;
		  // immune to knife damage
		  /*
		  CEntityPointer penWeapons= ((CPlayer*) penInflictor)->m_penWeapons;
		  if(((CPlayerWeapons*) penWeapons)->m_iCurrentWeapon==WEAPON_KNIFE)
		  {
			  fDamageAmmount= fDamageAmmount/10;
		  }*/
		}
  ...
  BOOL FillEntityStatistics(EntityStats *pes)
  {
	...
	case EYC_ELITE:    { pes->es_strName+=" Elite"; } break;
    }
    if (m_bInvisible) {
  ...
  virtual const CTFileName &GetComputerMessageName(void) const {
	...
	static DECLARE_CTFILENAME(fnmElite   , "Data\\Messages\\Enemies\\EyemanElite.txt");
    switch(m_EecChar) {
    default: ASSERT(FALSE);
    case EYC_SERGEANT: return fnmSergeant;
    case EYC_SOLDIER : return fnmSoldier;
	case EYC_ELITE   : return fnmElite;
    }
  };
 ...
 ************************************************************/
  Main(EVoid) {
    ...
    if (m_EecChar==EYC_SERGEANT) {
      SetHealth(90.0f * m_fHPMultiplier);
      m_fMaxHealth = 90.0f * m_fHPMultiplier;
      ...
      en_fDensity = 2000.0f;
    } else if (m_EecChar==EYC_SOLDIER) {
      SetHealth(60.0f * m_fHPMultiplier);
      m_fMaxHealth = 60.0f * m_fHPMultiplier;
      // damage/explode properties
      ...
      en_fDensity = 2000.0f;
    } else if (m_EecChar==EYC_ELITE) {
	  SetHealth(150.0f * m_fHPMultiplier);
	  m_fMaxHealth= 150.0f * m_fHPMultiplier;
      m_fBlowUpAmount = 190.0f;
      m_fBodyParts = 5;
      m_fBlowUpSize = 3.5f;
    ...
    if (m_EecChar==EYC_SERGEANT) {
	  ...
	  m_iScore= 2500;
	  SetModelMainTexture(TEXTURE_EYEMAN_ELITE);
	...
	if(m_EecChar==EYC_ELITE)
	{
      ...
      m_fAttackRunSpeed = FRnd()*2.0f + 30.0f;
      m_aAttackRotateSpeed = AngleDeg(1800.0f);
      m_fCloseRunSpeed = FRnd()*2.0f + 30.0f;
	...
	// no need to set up for elite because he can't fly
