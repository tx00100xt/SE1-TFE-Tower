...
properties:
  ...
  3 INDEX m_iGiveWeapons     "Give Weapons" 'W' = 0x3,  // default is colt+knife
 ...
 12 CEntityPointer m_penParent "Parent to Player",
...
functions:
  void Precache(void) {
    // we don't want to player to have only double colts with no single colt
    if( (m_iGiveWeapons & 0x04) && !(m_iGiveWeapons & 0x02) )
	{
	  m_iGiveWeapons |= 0x02;  // giving player a colt
	}

	// nor do we want only one launcher without the other
	if( (m_iGiveWeapons & 131072) || (m_iGiveWeapons & 256) )
	{
	  m_iGiveWeapons |= 131328;
	}
