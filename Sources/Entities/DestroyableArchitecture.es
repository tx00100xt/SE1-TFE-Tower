...
enum DestroyableArchitectureMaterialType {
  0 MAT_ARCHITECTURE_ROCK "Rock",
  1 MAT_ARCHITECTURE_WOOD "Wood",
  2 MAT_ARCHITECTURE_GLASS "Glass",
};
...
300 enum DestroyableArchitectureMaterialType m_ltMaterial "Debris Type" = MAT_ARCHITECTURE_ROCK,
...
200 model     MODEL_WOOD         "Models\\Effects\\Debris\\Wood01\\Wood.mdl",
201 texture   TEXTURE_WOOD       "Models\\Effects\\Debris\\Wood01\\Wood.tex",
202 model     MODEL_GLASS		 "Models\\Effects\\Debris\\Glass\\Glass.mdl",
203 texture   TEXTURE_GLASS		 "Models\\Effects\\Debris\\Glass\\Glass.tex",
  ...
  Main() {
    ...
    wait() {
      ...
      on (EDeath eDeath) : {
		...
		switch(m_ltMaterial) {
		  case MAT_ARCHITECTURE_ROCK:
			FinalModel= MODEL_STONE;
			FinalTexture= TEXTURE_STONE;
			FlightMultiplier= 1.0f;
			break;
		  case MAT_ARCHITECTURE_WOOD:
			FinalModel= MODEL_WOOD;
			FinalTexture= TEXTURE_WOOD;
			FlightMultiplier= 2.0f;
			break;
		  case MAT_ARCHITECTURE_GLASS:
			FinalModel= MODEL_GLASS;
			FinalTexture= TEXTURE_GLASS;
			FlightMultiplier= 4.0f;
			break;
		}
        ...
        for(INDEX iDebris = 0; iDebris<m_ctDebrises; iDebris++) {
          Debris_Spawn(this, this, FinalModel, FinalTexture, 0, 0, 0, IRnd()%4, 1.0f,
            FLOAT3D(FRnd()*0.8f*FlightMultiplier+0.1f,
			        FRnd()*0.8f*FlightMultiplier+0.1f,
					FRnd()*0.8f*FlightMultiplier+0.1f));
        }

        /*
		Old debris code
		Replaced with the above (copied from MovingBrush)
		Debris_Begin(EIBT_ROCK, DPT_NONE, BET_NONE, fEntitySize, FLOAT3D(1.0f,2.0f,3.0f),
          FLOAT3D(0,0,0), 1.0f+m_fCandyEffect/2.0f, m_fCandyEffect, m_colDebrises);
        for(INDEX iDebris = 0; iDebris<m_ctDebrises; iDebris++) {
