...
// reload weapon
event EReloadWeapon {};
// remove freezed weapon
event ERemoveFreezedWeapon {};

// fire and release secondary
event EFireSecondaryWeapon {};
...
// 17 WEAPON_NUKECANNON         "",
 17 WEAPON_SPIKEBALLLAUNCHER  "",       // this is actually the grenade launcher
 18 WEAPON_LAST               "",
};
...
#define SPIKEBALL_THROW_DIVISOR      0.1f
#define SPIKEBALL_WIDE_THROW_DIVISOR 0.02f

#define SPIKEBALL_MINIMUM_THROW 0.0f
#define SPIKEBALL_MINIMUM_THROW_WIDE 0.0f
/*
#if BUILD_TEST
...
void CPlayerWeapons_Precache(ULONG ulAvailable)
{
  ...
  if ( ulAvailable&(1<<(WEAPON_MINIGUN-1)) ) {
	...
	pdec->PrecacheSound(SOUND_MINIGUN_FIRE_HIGH);
  }
  ...
  if ( ulAvailable&(1<<(WEAPON_GRENADELAUNCHER-1)) ||
	   ulAvailable&(1<<(WEAPON_SPIKEBALLLAUNCHER-1)) 
	  ) {
	...
	pdec->PrecacheClass(CLASS_PROJECTILE, PRT_SPIKEBALL);
  }
...
*/
  ...
  if ( ulAvailable&(1<<(WEAPON_LASER-1)) ) {
	...
	pdec->PrecacheSound(SOUND_DEATHRAY_FIRE);
...
properties:
 ...
 11 INDEX m_iAvailableWeapons = 0x00,   // avaible weapons (ADD) POSSIBLE CRASH
...
// 55 INDEX m_iMaxNukeBalls   = MAX_NUKEBALLS,
 56 INDEX m_iSpikeballs     = 0,
 57 INDEX m_iMaxSpikeballs  = MAX_SPIKEBALLS,
...
300 BOOL m_bSecondaryFireWeapon = FALSE,   // secondary fire
301 CEntityPointer m_penRocketLauncherHoming,  // target for rocket launcher
302 BOOL m_bDisableDeathray= FALSE, // whether deathray has been used
303 FLOAT m_fMinigunHeatFactor= 0.0f, // damage dealt by super minigun
...
147 sound   SOUND_DEATHRAY_FIRE         "Models\\Weapons\\Laser\\Sounds\\_Deathray.wav",
/*
// ************** GHOSTBUSTER **************
...
281 sound   SOUND_MINIGUN_FIRE_HIGH		"Models\\Weapons\\MiniGun\\Sounds\\FireHighPitch.wav",
...
//    plan.m_fRecoilSpeed += wpn_fRecoilSpeed[m_iCurrentWeapon];
  ...
  {
	return m_bSecondaryFireWeapon && !m_bChangeWeapon;
  }
...
//    if( iWeaponData==WEAPON_NUKECANNON) { iWeaponData = WEAPON_IRONCANNON; }
  ...
  void WeaponMovingOffset(FLOAT3D &plPos)
  {
    ...
    if( (m_iCurrentWeapon == WEAPON_GRENADELAUNCHER) ||
		(m_iCurrentWeapon == WEAPON_SPIKEBALLLAUNCHER) )
    {
...
//        AddAttachmentToModel(this, m_moWeapon, CANNON_ATTACHMENT_LIGHT, MODEL_CN_LIGHT, TEXTURE_CANNON, TEX_REFL_LIGHTMETAL01, TEX_SPEC_MEDIUM, 0);
  ...
  void FireOneBullet(FLOAT fX, FLOAT fY, FLOAT fRange, FLOAT fDamage) {
    ...
    ((CBullet&)*penBullet).LaunchBullet(TRUE, 0, TRUE);
    ((CBullet&)*penBullet).DestroyBullet();
  };

  // fires DA DEATHRAY!!
  // muhahaha
  void FireDeathray(FLOAT fX, FLOAT fY, FLOAT fRange, FLOAT fDamage) {
    PrepareBullet(fX, fY, fDamage);
    ((CBullet&)*penBullet).CalcTarget(fRange);
    ((CBullet&)*penBullet).m_fBulletSize = 0.1f;
    ((CBullet&)*penBullet).LaunchBullet(FALSE, 2, FALSE);
    ((CBullet&)*penBullet).DestroyBullet();
  };
  ...
  void FireBullets(FLOAT fX, FLOAT fY, FLOAT fRange, FLOAT fDamage, INDEX iBullets,
    ...
    for (iSlug=0; iSlug<iBullets; iSlug++) {
      ...
      ((CBullet&)*penBullet).LaunchBullet(iSlug<2, 0, TRUE);
    }
    ((CBullet&)*penBullet).DestroyBullet();
  };
  ...
  // fire spikeball- includes random value, for multiple balls
  void FireSpikeball(INDEX iPower, FLOAT3D rndValue) {
    // spikeball start position
    CPlacement3D plSpikeball;
    CalcWeaponPosition(
      FLOAT3D(wpn_fFX[WEAPON_SPIKEBALLLAUNCHER],wpn_fFY[WEAPON_SPIKEBALLLAUNCHER], 0), 
      plSpikeball, TRUE);

	plSpikeball.pl_OrientationAngle+= rndValue;

    // create spikeball
    CEntityPointer penSpikeball = CreateEntity(plSpikeball, CLASS_PROJECTILE);
    // init and launch spikeball
    ELaunchProjectile eLaunch;
    eLaunch.penLauncher = m_penPlayer;
    eLaunch.prtType = PRT_SPIKEBALL;
    eLaunch.fSpeed = 20.0f+iPower*5.0f;
    penSpikeball->Initialize(eLaunch);
  };

  // fire a random spikeball
  void FireRandomSpikeball(INDEX iPower)
  {
    float Direction;
    if(IRnd() %2 ==0)
	{
	  Direction= FRnd()/SPIKEBALL_THROW_DIVISOR;
      if (Direction<SPIKEBALL_MINIMUM_THROW) { Direction= SPIKEBALL_MINIMUM_THROW; }
	}
	else
	{
	  Direction= -FRnd()/SPIKEBALL_THROW_DIVISOR;
    ...
    FireSpikeball(iPower, FLOAT3D( Direction, 0.0f, 0.0f));
	return;
  }

  // fire a random spikeball
  // this is the "wide" variant, for alternate fire
  void FireRandomSpikeballWide(INDEX iPower)
  {
    float Direction;
    if(IRnd() %2 ==0)
	{
	  Direction= FRnd()/SPIKEBALL_WIDE_THROW_DIVISOR;
	  if (Direction<SPIKEBALL_MINIMUM_THROW) { Direction= SPIKEBALL_MINIMUM_THROW; }
	}
	else
	{
...
//    }
  ...
  void ClearWeapons(void) {
    // give/take weapons
    m_iAvailableWeapons = 0x00;  // (ADD) possible crash here
    m_iColtBullets = 6;
    m_iBullets = 0;
    m_iShells = 0;
    m_iRockets = 0;
...
//    m_iMaxNukeBalls   = ClampUp((INDEX) ceil(MAX_NUKEBALLS*fModifier),    INDEX(999));
	...
	m_iMaxSpikeballs  = ClampUp((INDEX) ceil(MAX_SPIKEBALLS*fModifier), INDEX(999));
...
//    if( iTakeAmmo & (1<<AMMO_NUKEBALLS))     {m_iNukeBalls  = 0;}
	...
	if( iTakeAmmo & (1<<AMMO_SPIKEBALLS))    {m_iSpikeballs = 0;}
...
//    case WEAPON_NUKECANNON:      return m_iNukeBalls;
	  case WEAPON_SPIKEBALLLAUNCHER: return m_iSpikeballs;
    }
    return 0;
  };
...
//    case WEAPON_NUKECANNON:      return m_iMaxNukeBalls;
	  case WEAPON_SPIKEBALLLAUNCHER: return m_iMaxSpikeballs;
    }
    return 0;
  };
  ...
  void CheatGiveAll(void) {
	...
	m_iSpikeballs = m_iMaxSpikeballs;
    // precache eventual new weapons
    Precache();
  };
...
//    m_iNukeBalls   = ClampUp(m_iNukeBalls,    m_iMaxNukeBalls);
	m_iSpikeballs  = ClampUp(m_iSpikeballs, m_iMaxSpikeballs);
  }
  ...
  void AddDefaultAmmoForWeapon(INDEX iWeapon, FLOAT fMaxAmmoRatio)
  {
    ...
    switch (iWeapon) {
      ...
      case WEAPON_GRENADELAUNCHER:
        ...
        break;
	  case WEAPON_SPIKEBALLLAUNCHER:
		iAmmoPicked= Max(5.0f, m_iMaxSpikeballs*fMaxAmmoRatio);
		m_iSpikeballs+= iAmmoPicked;
		AddManaToPlayer(iAmmoPicked*AV_SPIKEBALLS*fMaxAmmoRatio);
		break;
/*
      case WEAPON_PIPEBOMB:
        iAmmoPicked = Max(5.0f, m_iMaxGrenades*fMaxAmmoRatio);
...
//      case WIT_GHOSTBUSTER: Ewi.iWeapon = WEAPON_GHOSTBUSTER; break;
	...
	if(Ewi.iWeapon == WEAPON_GRENADELAUNCHER)
	{
		Ewi.iWeapon= WEAPON_SPIKEBALLLAUNCHER;
		m_iAvailableWeapons |= 1<<(Ewi.iWeapon-1);
	}
//    m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASK;
...
//        break;
	...
	// decide which ammunition to give to player
	if(!Ewi.bGiveAlternateAmmo && Ewi.iWeapon==WEAPON_SPIKEBALLLAUNCHER)
	{
		Ewi.iWeapon= WEAPON_GRENADELAUNCHER;
	}
  ...
  BOOL ReceiveAmmo(const CEntityEvent &ee) {
    ...
    switch (Eai.EaitType) {
	  ...
	  // spikeballs
	  case AIT_SPIKEBALLS:
		if (m_iSpikeballs>=m_iMaxSpikeballs) { m_iSpikeballs = m_iMaxSpikeballs; return FALSE; }
		m_iSpikeballs+= Eai.iQuantity;
		((CPlayer&)*m_penPlayer).ItemPicked(TRANS("Spikeballs"), Eai.iQuantity);
		AddManaToPlayer(Eai.iQuantity*AV_SPIKEBALLS*MANA_AMMO);
		break;
      // electicity
...
//        (eapi.iNapalm>0 && m_iNapalm<m_iMaxNapalm) ||
		...
		(eapi.iSpikeballs>0 && m_iSpikeballs<m_iMaxSpikeballs)
/*        || (eapi.iNukeBalls>0 && m_iNukeBalls<m_iMaxNukeBalls) */)
    {
	  ...
	  m_iSpikeballs+=eapi.iSpikeballs;
...
//      m_iNukeBalls+=eapi.iNukeBalls;
	  ...
	  if( eapi.iSpikeballs != 0) {strMessage.PrintF("%s %d %s,", (const char*)strMessage, eapi.iGrenades, TRANS("Spikeballs"));}
...
//      if( eapi.iNukeBalls != 0) {strMessage.PrintF("%s %d %s,", strMessage, eapi.iNukeBalls, TRANS("Nuke balls"));}
  ...
  WeaponType GetStrongerWeapon(INDEX iWeapon) {
    switch(iWeapon) {
	  ...
	  case 6: return WEAPON_SPIKEBALLLAUNCHER;
      case 7: return WEAPON_GRENADELAUNCHER;
...
//      case 8: return WEAPON_GHOSTBUSTER;
  ...
  INDEX GetSelectedWeapon(WeaponType EwtSelectedWeapon) {
    switch(EwtSelectedWeapon) {
	  ...
	  case WEAPON_SPIKEBALLLAUNCHER: return 6;
      case WEAPON_GRENADELAUNCHER: return 7;
...
//      case WEAPON_FLAMER: return 7;
  ...
  WeaponType GetAltWeapon(WeaponType EwtWeapon) {
    switch (EwtWeapon) {
      ...
      case WEAPON_GRENADELAUNCHER: return WEAPON_SPIKEBALLLAUNCHER;
	  case WEAPON_SPIKEBALLLAUNCHER: return WEAPON_GRENADELAUNCHER;
...
//        WeaponSelectOk(WEAPON_NUKECANNON)||
		...
		WeaponSelectOk(WEAPON_SPIKEBALLLAUNCHER)||
...
//        WeaponSelectOk(WEAPON_FLAMER)||
  ...
  BOOL HasAmmo(WeaponType EwtWeapon) {
    switch (EwtWeapon) {
	  ...
	  case WEAPON_SPIKEBALLLAUNCHER: return (m_iSpikeballs>0);
...
//      case WEAPON_NUKECANNON:
  ...
  FLOAT SpikeballLauncherBoring(void) {
    // play boring anim
    m_moWeapon.PlayAnim(GRENADELAUNCHER_ANIM_WAIT2, AOF_SMOOTHCHANGE);
    return m_moWeapon.GetAnimLength(GRENADELAUNCHER_ANIM_WAIT2);
  };
  ...
  */
  ...
  WeaponType PrimaryToSecondary(WeaponType wt)
  {
    if (wt==WEAPON_DOUBLECOLT) {
	  ...
	  return WEAPON_SPIKEBALLLAUNCHER;
...
/*
  ...
  WeaponType SecondaryToPrimary(WeaponType wt)
  {
	...
	} else if (wt==WEAPON_SPIKEBALLLAUNCHER) {
	  return WEAPON_GRENADELAUNCHER;
/*    } else if (wt==WEAPON_GRENADELAUNCHER) {
...
//        wt = WEAPON_NUKECANNON;
        wt = WEAPON_SPIKEBALLLAUNCHER;
      }
//      if (wt>WEAPON_NUKECANNON) {
      if (wt>WEAPON_SPIKEBALLLAUNCHER) {
        wt = WEAPON_KNIFE;
      }
      if (wt==wtOrg) {
        break;
      ...
      wait() {
        on (EBegin) : {
          ...
          switch (m_iCurrentWeapon) {
			...
			case WEAPON_SPIKEBALLLAUNCHER: call FireSpikeballLauncher(); break;
...
//      case WEAPON_GHOSTBUSTER: { jump GhostBusterStop(); break; }
  ...
  FireSecondaryColt() {
	...
	// fire SUPERbullet :)
    if ( ((CPlayer&)*m_penPlayer).m_bUnderRage )
	{
	  FireOneBullet(wpn_fFX[WEAPON_COLT], wpn_fFY[WEAPON_COLT], 500.0f, m_iColtBullets*15.0f);
	}
    else
	{
	  FireOneBullet(wpn_fFX[WEAPON_COLT], wpn_fFY[WEAPON_COLT], 500.0f, m_iColtBullets*5.0f);
	}

    if(_pNetwork->IsPlayerLocal(m_penPlayer)) {IFeel_PlayEffect("Colt_fire");}
	DoRecoil();
	SpawnRangeSound(90.0f);
	m_iColtBullets= 0;
  ...
  // null secondary fire
  // for weapons which doesn't support it
  NullSecondaryFire()
  {
	  autowait(0.25f);
      return EEnd();
  ...
  /**********************************
     Beginning of secondary minigun
   **********************************/
  MiniGunSecondarySpinUp() {
    // steady anim
    ...
    while (m_aMiniGunSpeed<MINIGUN_FULLSPEED*2 && HoldingSecondaryFire()) {
      // every tick
    ...
    if (!HoldingSecondaryFire()) {
      // start spindown
      jump MiniGunSecondarySpinDown();
    }
    // start fireing
    jump MiniGunSecondaryFire();
  }

  MiniGunSecondaryFire() {
    // spinning sound
    CPlayer &pl = (CPlayer&)*m_penPlayer;
    ...
    // if firing
    if(HoldingSecondaryFire() && m_iBullets>1) {
      // play fire sound
      PlaySound(pl.m_soWeapon0, SOUND_MINIGUN_FIRE_HIGH, SOF_3D|SOF_LOOP|SOF_VOLUMETRIC);
      ...
      if (m_iBullets>1) {
        // fire a bullet
	      ...
	      if ( ((CPlayer&)*m_penPlayer).m_bUnderRage )
		  {
		      ...
		      750.0f, 60.0f, (GetSP()->sp_bCooperative) ? 0.01f : 0.03f, 
			  ( (GetSP()->sp_bCooperative) ? 0.5f : 0.0f));
		  }
		  else
		  {
			  FireMachineBullet(wpn_fFX[WEAPON_MINIGUN], wpn_fFY[WEAPON_MINIGUN],
		      750.0f, 20.0f, (GetSP()->sp_bCooperative) ? 0.01f : 0.03f, 
			  ( (GetSP()->sp_bCooperative) ? 0.5f : 0.0f));
		  }

		m_fMinigunHeatFactor++;
		if(m_fMinigunHeatFactor>40)
		{
			//InflictDirectDamage(m_penPlayer, this, DMT_CLOSERANGE, m_fMinigunHeatFactor/20, FLOAT3D(0,0,0), FLOAT3D(0,0,0));
		}
		  
        DoRecoil();
        SpawnRangeSound(60.0f);
        DecAmmo(m_iBullets, 2);
        SetFlare(0, FLARE_ADD);
  ...
  MiniGunSecondarySpinDown() {
    ...
    while ( m_aMiniGunSpeed>0 && (!HoldingSecondaryFire() || m_iBullets<=0)) {
	  if(HoldingFire() && m_iBullets>0)
	  {
		  jump MiniGunSpinUp();
	  }
      autowait(MINIGUN_TICKTIME);
	  m_fMinigunHeatFactor-= 0.3f;
	  if(m_fMinigunHeatFactor<0) { m_fMinigunHeatFactor=0; }
      // spin
      m_aMiniGunLast = m_aMiniGun;
      m_aMiniGun+=m_aMiniGunSpeed*MINIGUN_TICKTIME;
    ...
    // if should fire
    if (HoldingSecondaryFire() && m_iBullets>0) {
      // start spinup
      jump MiniGunSecondarySpinUp();
    }
    ...
    if (m_iBullets<=0) { 
      ...
      while(HoldingSecondaryFire() && m_iBullets<=0) {
        autowait(0.1f);
      }
      if (m_iBullets<=0) {
        // select new weapon
		m_fMinigunHeatFactor= 0;
        SelectNewWeapon(); 
      }
    }
  ...
  /****************************
     End of secondary minigun
   ****************************/
  // main procedure
  FireSecondary()
  {
    //CMusicHolder *pmh = (CMusicHolder *)&* (((CPlayer&) *m_penPlayer).m_penMainMusicHolder);

	//if(pmh!=NULL) {
	//  if(pmh->m_bUseSecondaryFire)
	//  {
        CPlayer &pl = (CPlayer&)*m_penPlayer;
        PlaySound(pl.m_soWeapon0, SOUND_SILENCE, SOF_3D|SOF_VOLUMETRIC);      // stop possible sounds
        // force ending of weapon change
        ...
        if (m_iCurrentWeapon==WEAPON_MINIGUN) { jump MiniGunSecondarySpinUp(); }
	  ...
	  // clear last lerped bullet position
        ...
        while (HoldingSecondaryFire() && m_bHasAmmo) {
          ...
          wait() {
            on (EBegin) : {
              // fire one shot
     	      // this is where I code the new functions
		      // continous weapons are disabled here
              switch (m_iCurrentWeapon) {
                case WEAPON_KNIFE: call NullSecondaryFire(); break;
                case WEAPON_COLT: call FireSecondaryColt(); break;
                case WEAPON_DOUBLECOLT: call FireSecondaryDoubleColt(); break;
                case WEAPON_SINGLESHOTGUN: call NullSecondaryFire(); break;
                case WEAPON_DOUBLESHOTGUN: call FireSecondaryDoubleShotgun(); break;
                case WEAPON_TOMMYGUN: call NullSecondaryFire(); break;
   	  	  	    case WEAPON_MINIGUN: call NullSecondaryFire(); break;
                case WEAPON_IRONCANNON: call NullSecondaryFire(); break;
                case WEAPON_ROCKETLAUNCHER: call FireSecondaryRocketLauncher(); break;
 			    case WEAPON_SPIKEBALLLAUNCHER: call NullSecondaryFire(); break;
                case WEAPON_GRENADELAUNCHER: call NullSecondaryFire(); break;
                case WEAPON_LASER: call FireSecondaryLaser(); break;
                default: ASSERTALWAYS("Unknown weapon.");
			  }
              resume;
			}
  	    ...
  	    if(m_iCurrentWeapon==WEAPON_MINIGUN) { jump MiniGunSecondarySpinDown(); }
        m_bSecondaryFireWeapon= FALSE; //NEW #####
	    jump Idle();
/*	  }
	  else
	  {
		m_bSecondaryFireWeapon= FALSE;
		jump Idle();
	  }
	}
	else
...
/*
  ...
  FireColt() {
    ...
    if ( ((CPlayer&)*m_penPlayer).m_bUnderRage )
	{
		FireOneBullet(wpn_fFX[WEAPON_COLT], wpn_fFY[WEAPON_COLT], 500.0f, 30.0f);
	}
	else
	{
  ...
  FireDoubleColt() {
    ...
    if ( ((CPlayer&)*m_penPlayer).m_bUnderRage )
	{
		FireOneBullet(wpn_fFX[WEAPON_DOUBLECOLT], wpn_fFY[WEAPON_DOUBLECOLT], 500.0f, 30.0f);
	}
	else
	{
    ...
    if (m_iShells>0) {
      ...
      if ( ((CPlayer&)*m_penPlayer).m_bUnderRage )
	  {
		  FireBullets(wpn_fFX[WEAPON_SINGLESHOTGUN], wpn_fFY[WEAPON_SINGLESHOTGUN], 
			500.0f, 30.0f, 7, afSingleShotgunPellets, 0.2f, 0.03f);
	  }
	  else
	  {
		  FireBullets(wpn_fFX[WEAPON_SINGLESHOTGUN], wpn_fFY[WEAPON_SINGLESHOTGUN], 
    ...
    if (m_iShells>1) {
      ...
      if ( ((CPlayer&)*m_penPlayer).m_bUnderRage )
	  {
		  FireBullets(wpn_fFX[WEAPON_DOUBLESHOTGUN], wpn_fFY[WEAPON_DOUBLESHOTGUN], 
			500.0f, 30.0f, 14, afDoubleShotgunPellets, 0.3f, 0.03f);
	  }
	  else
	  {
		  FireBullets(wpn_fFX[WEAPON_DOUBLESHOTGUN], wpn_fFY[WEAPON_DOUBLESHOTGUN], 
    ...
    while (HoldingFire()) {
      ...
      if (m_iBullets>0) {
        // fire a bullet
		  ...
		  {
			  FireMachineBullet(wpn_fFX[WEAPON_MINIGUN], wpn_fFY[WEAPON_MINIGUN],
		      750.0f, 30.0f, (GetSP()->sp_bCooperative) ? 0.01f : 0.03f, 
			  ( (GetSP()->sp_bCooperative) ? 0.5f : 0.0f));
		  }
		  else
		  {
  ...
  FireSecondaryRocketLauncher() {
    ...
    if( crRay.cr_penHit!=NULL)
    {
	  ...
	  {
		  // set up the target
		  m_penRocketLauncherHoming= pen;

		  if( IsOfClass(pen, "Player") )
		  {
			  m_strLastTarget= TRANS("Achieved Lock on Target Player");
		  }
		  else
		  {
			  m_strLastTarget.PrintF(TRANS("Achieved Lock on %s"), (const char*) ((CEnemyBase*)pen)->GetIngameName());
		  }
		  m_tmLastTarget = _pTimer->CurrentTick()+0.5f;
	  }
	  else
	  {
		  m_penRocketLauncherHoming= NULL;
	  }
	}
	else
	{
	...
	autowait(0.1f);  // so we won't get an infinite event loop
    return EEnd();
  }
  ...
  FireRocketLauncher() {
    ...
    if (m_iRockets>0) {
      ...
      m_moWeapon.PlayAnim(ROCKETLAUNCHER_ANIM_FIRE, 0);
		if(m_penRocketLauncherHoming!=NULL)
		{
		  if(!(m_penRocketLauncherHoming->GetFlags()&ENF_ALIVE))
		  {
			  m_penRocketLauncherHoming= NULL;
		  }
		}
...
// ***************** FIRE SPIKEBALL LAUNCHER *****************
  FireSpikeballLauncher()
  {
    TM_START = _pTimer->CurrentTick();
    // remember time for spring release
    F_TEMP = _pTimer->CurrentTick();
    ...
    // release spring and fire three spikeballs
    if (m_iSpikeballs>0)
    {
      // fire spikeball
      INDEX iPower = INDEX((_pTimer->CurrentTick()-F_TEMP)/_pTimer->TickQuantum);

	  FireRandomSpikeball(iPower);
	  FireRandomSpikeball(iPower);
	  FireRandomSpikeball(iPower);
	  FireRandomSpikeball(iPower);
	  FireRandomSpikeball(iPower);
      ...
      ASSERTALWAYS("SpikeballLauncher - Auto weapon change not working.");
      m_bFireWeapon = m_bHasAmmo = FALSE;
    }
  ...
  FireSecondarySpikeballLauncher()
  {
    TM_START = _pTimer->CurrentTick();
    // remember time for spring release
    F_TEMP = _pTimer->CurrentTick();
    ...
    m_fWeaponDrawPower = 0.0f;
    m_tmDrawStartTime = _pTimer->CurrentTick();
    while (HoldingSecondaryFire() && ((_pTimer->CurrentTick()-TM_START)<0.75f) )
    {
      autowait(_pTimer->TickQuantum);
      INDEX iPower = INDEX((_pTimer->CurrentTick()-TM_START)/_pTimer->TickQuantum);
      F_OFFSET_CHG = 0.125f/(iPower+2);
    ...
    if (m_iSpikeballs>0)
    {
	  ...
	  FireRandomSpikeballWide(iPower);
	  FireRandomSpikeballWide(iPower);
	  FireRandomSpikeballWide(iPower);
	  FireRandomSpikeballWide(iPower);
	  FireRandomSpikeballWide(iPower);
  ...
  FireSecondaryDoubleShotgun()
  {
    ...
    if( crRay.cr_penHit!=NULL)
    {
      ...
      if( pen->GetFlags()&ENF_ALIVE)
	  {
		  if( IsOfClass(pen, "Player") )
		  {
			  m_strLastTarget.PrintF(TRANS("Player! Life is %d, Armor is %d"),
									 (SLONG)ceil(((CPlayer*)pen)->GetHealth()),
									 (SLONG)ceil(((CPlayer*)pen)->m_fArmor ));
		  }
		  else
		  {
			  m_strLastTarget.PrintF(TRANS("%s! Life left is %d"),
				  (const char*)((CEnemyBase*)pen)->GetIngameName(),
				  (SLONG)ceil(((CEnemyBase*)pen)->GetHealth()));
		  }
		  m_tmLastTarget = _pTimer->CurrentTick()+1.5f;
	  }
	}
  ...
  FireSecondaryLaser()
  {
	float requiredDamage=1.0f;
	float requiredWait= 4.0f;
    CPlacement3D plCrosshair;
    FLOAT fFX = wpn_fFX[m_iCurrentWeapon];  // get weapon firing position
    FLOAT fFY = wpn_fFY[m_iCurrentWeapon];
    if (GetPlayer()->m_iViewState == PVT_3RDPERSONVIEW) {
    ...
    if( crRay.cr_penHit!=NULL)
    {
      ...
      if( pen->GetFlags()&ENF_ALIVE)
	  {
		  if( IsOfClass(pen, "Player") )
		  {
			  requiredDamage= ((CPlayer*)pen)->GetHealth() + 
				              ((CPlayer*)pen)->m_fArmor;
			  requiredWait= requiredDamage/20;
		  }
		  else
		  {
			  requiredDamage= ((CEnemyBase*)pen)->GetHealth();
			  requiredWait= requiredDamage/40;
		  }
		  if(requiredWait<0.1f) { requiredWait= 0.1f; }
	  }
	}

	if(m_iElectricity>=(requiredDamage/2))
	{
		m_iElectricity-= (INDEX) ceil(requiredDamage/2);
	  
		CPlayer &pl = (CPlayer&)*m_penPlayer;

		FireDeathray(wpn_fFX[WEAPON_LASER], wpn_fFY[WEAPON_LASER], 500.0f, requiredDamage);
        PlaySound(pl.m_soWeapon0, SOUND_DEATHRAY_FIRE, SOF_3D|SOF_VOLUMETRIC);
        if(_pNetwork->IsPlayerLocal(m_penPlayer)) {IFeel_PlayEffect("Laser_fire");}

		CModelObject *pmo;
		pmo = &(m_moWeapon.GetAttachmentModel(LASER_ATTACHMENT_RIGHTUP)->amo_moModelObject);
		pmo->PlayAnim(BARREL_ANIM_FIRE, 0);

		pmo = &(m_moWeapon.GetAttachmentModel(LASER_ATTACHMENT_LEFTUP)->amo_moModelObject);
		pmo->PlayAnim(BARREL_ANIM_FIRE, 0);

		pmo = &(m_moWeapon.GetAttachmentModel(LASER_ATTACHMENT_RIGHTDOWN)->amo_moModelObject);
		pmo->PlayAnim(BARREL_ANIM_FIRE, 0);

		pmo = &(m_moWeapon.GetAttachmentModel(LASER_ATTACHMENT_LEFTDOWN)->amo_moModelObject);
		pmo->PlayAnim(BARREL_ANIM_FIRE, 0);

        if (m_iElectricity<=0) { SelectNewWeapon(); }

		m_strLastTarget.PrintF(TRANS("Reloading deathray, please hold"));
	    m_tmLastTarget = _pTimer->CurrentTick()+requiredWait;
		m_bDisableDeathray= TRUE;
		autowait(requiredWait);
		m_bDisableDeathray= FALSE;
	}
  ...
  Idle() {
    ...
    wait() {
      on (EBegin) : {
		...
		// secondary fire pressed
		if(m_bSecondaryFireWeapon) {
		  jump FireSecondary();
		}
        resume;
      }
