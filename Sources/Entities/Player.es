...
event ECameraStart {
  ...
  BOOL HaltPlayer,  // whether the stop the player's movement
};
...
#define PLACT_FIRESECONDARY		  (1L<<1)
#define PLACT_RELOAD              (1L<<2)
#define PLACT_WEAPON_NEXT         (1L<<3)
#define PLACT_WEAPON_PREV         (1L<<4)
#define PLACT_WEAPON_FLIP         (1L<<5)
#define PLACT_USE                 (1L<<6)
#define PLACT_COMPUTER            (1L<<7)
#define PLACT_3RD_PERSON_VIEW     (1L<<8)
#define PLACT_CENTER_VIEW         (1L<<9)
...
//  CPrintF("compose: prescan %d, x:%g\n", bPreScan, paAction.pa_aRotation(1));
  ...
  if(pctlCurrent.bFireSecondary) paAction.pa_ulButtons |= PLACT_FIRESECONDARY;
...
properties:
 ...
 299 BOOL m_bOnCameraHaltPlayer= FALSE,
 300 FLOAT m_tmRageStarted = 0.0f,			  // when did the last rage potion was taken
 301 BOOL m_bUnderRage= FALSE,				  // whether the player is affected by rage
 302 FLOAT m_tmRageWillEnd= 0.0f,             // how long to keep the rage going
 
 303 FLOAT m_tmRegenStarted= 0.0f,     // when did the last regeneration potion was taken
 304 BOOL m_bUnderRegen= FALSE,        // whether the player is affected by regeneration
 305 FLOAT m_tmRegenWillEnd= 0.0f,     // how long to keep regeneration going

 306 CEntityPointer m_penPrivateParent, // this player's parent to player entity
 ...
 ************************************************************/
  ...
  void PostMoving(void)
  {
    ...
    // if in tourist mode, or using a regeneration potion
 ...
 ************************************************************/
                      ...
                      FLOAT fDamageAmmount, const FLOAT3D &vHitPoint, const FLOAT3D &vDirection)
  {
    ...
    // if any damage
    if( fSubHealth>0) { 
      // if camera is active and not taking over the controls
      if (m_penCamera!=NULL && m_bOnCameraHaltPlayer) {
 ...
 ************************************************************/
  ...
  BOOL ReceiveItem(const CEntityEvent &ee)
  {
	...
	// *********** POTION **********
	if ( ee.ee_slEvent == EVENTCODE_EPotion)
	{
		switch( ((EPotion&)ee).iPotion )
		{
		  case PTN_RAGE:
			m_tmRageStarted= 0;
			m_tmRageWillEnd= ((EPotion&)ee).fPotionTime;
			m_bUnderRage= TRUE;
			ItemPicked(TRANS("Potion of Rage"), 0);
			break;

		  case PTN_REGEN:
			m_tmRegenStarted= 0;
			m_tmRegenWillEnd= ((EPotion&)ee).fPotionTime;
			m_bUnderRegen= TRUE;
			ItemPicked(TRANS("Potion of Regeneration"), 0);
			break;
		}
	  return TRUE;
	}
...
//    CPrintF("---APPLY: %g\n", paOriginal.pa_aRotation(1));
    ...
    if (GetFlags() & ENF_ALIVE) {
		...
		if(m_bUnderRage)  // updating the rage counter
		{
			m_tmRageStarted+= 0.05f;
			if(m_tmRageStarted>=m_tmRageWillEnd) { m_bUnderRage= FALSE; }
		}

		if(m_bUnderRegen) // updating the regeneration counter
		{
			m_tmRegenStarted+= 0.05f;
			if(m_tmRegenStarted>=m_tmRegenWillEnd) { m_bUnderRegen= FALSE; }
		}
  ...
  void ActiveActions(const CPlayerAction &paAction)
  {
    ...
    // normal mode
	  ...
	  // check for ladder
	  if(en_pbpoStandOn!=NULL && en_pbpoStandOn->bpo_bppProperties.bpp_ubSurfaceType==SURFACE_LADDER)
	  {
	    SetPhysicsFlags(GetPhysicsFlags() | EPF_STICKYFEET);
	  }	
  ...
  void ButtonsActions( CPlayerAction &paAction)
  {
	...
	if(ulNewButtons&PLACT_FIRESECONDARY) {
	  ((CPlayerWeapons&)*m_penWeapons).SendEvent(EFireSecondaryWeapon());
	}
	// if secondary fire is released
	if(ulReleasedButtons&PLACT_FIRESECONDARY) {
 ...
 ************************************************************/
  void InitializePlayer() {
	...
	// copying and adding the player parent
	if(m_penPrivateParent!= NULL)
	{
	  CEntity *pen= NULL;

	  // spawning to a random location
	  pen= GetWorld()->CopyEntityInWorld(*m_penPrivateParent,
	  ...
	  // setting its parent to the player
	  pen->SetParent(this);

	  // and remembering what is our parent
      m_penPrivateParent= pen;
	}
  };
...
void TeleportPlayer(enum WorldLinkType EwltType) 
  {
    ...
    if (m_ulFlags&PLF_RESPAWNINPLACE) {
	  ...
	  // i_might_want_to_add_playeR_parent_here
    ...
    // if start marker is found
	  ...
	  // locate the player parent
	  CPlayerParent *penPlayerParent= (CPlayerParent*)&*CpmStart.m_penParent;
	  if(penPlayerParent!= NULL)
	  {
		m_penPrivateParent= penPlayerParent;
	  }
 ...
 ************************************************************/
  Death(EDeath eDeath) {
	...
	((CPlayerWeapons&)*m_penWeapons).SendEvent(EReleaseSecondaryWeapon());
	((CPlayerWeapons&)*m_penWeapons).SendEvent(ERemoveFreezedWeapon());
	...
	// remove the player parent here
	if(m_penPrivateParent!= NULL)
	{
      ((CPlayerParent&)*m_penPrivateParent).SendEvent(EPlayerDied());
	}
  ...
  Main(EVoid evoid)
  {
    ...
    wait() {
      ...
      on (ECameraStart eStart) : {
        ...
        // stop player
        if (m_penActionMarker==NULL && eStart.HaltPlayer) {
          SetDesiredTranslation(FLOAT3D(0.0f, 0.0f, 0.0f));
          SetDesiredRotation(ANGLE3D(0.0f, 0.0f, 0.0f));
        }
        // stop firing
		if(eStart.HaltPlayer)
		{
          ((CPlayerWeapons&)*m_penWeapons).SendEvent(EReleaseWeapon());
		}

		m_bOnCameraHaltPlayer= eStart.HaltPlayer;
        resume;
      }
